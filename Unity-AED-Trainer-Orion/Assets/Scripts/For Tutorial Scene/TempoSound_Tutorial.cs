using UnityEngine;
using System.Collections;

public class TempoSound_Tutorial : MonoBehaviour {


    AudioSource AudioSource20;
    public bool isTempoPlay = false;

    //他クラスから継承
    public bool _isTempoSoundLoop;
    CPRAudio_Tutorial _cprAudio_tutorial;

    void Start()
    {
        _cprAudio_tutorial = GameObject.Find("CPRAudio").GetComponent<CPRAudio_Tutorial>();

        AudioSource[] audioSources = GetComponents<AudioSource>();
        AudioSource20 = audioSources[0];
    }

    void Update()
    {
        _isTempoSoundLoop = _cprAudio_tutorial.isTempoSoundLoop; //_isTempoSoundLoopの中身をCPRAudio.isTempoSoundLoopから参照して更新する用

        //電気ショックが終わったら実行される
        if (_isTempoSoundLoop == true && isTempoPlay == false && FlagManager.Instance.flags[7] == true)
        {
            double looptime = 0.5;
            AudioSource20.PlayScheduled(looptime);
            AudioSource20.loop = true;
            isTempoPlay = true;

        }
        if (_isTempoSoundLoop == false)
        {
            AudioSource20.loop = false;
            isTempoPlay = false;
        }
    }
}